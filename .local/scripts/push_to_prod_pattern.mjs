...
async function main() {
  ...
  console.log(`Scanning local Redis for keys matching '${pattern}'...`);
  let cursor = '0';
  const keys = [];
  do {
    const [nextCursor, batch] = await local.scan(cursor, 'MATCH', pattern, 'COUNT', 1000);
    cursor = nextCursor;
    keys.push(...batch);
  } while (cursor !== '0');

  console.log(`Found ${keys.length} keys in local Redis matching '${pattern}'. Reading and pushing in batches of 100...`);

  if (keys.length === 0) {
    console.log('No keys to push. Exiting.');
    process.exit(0);
  }
