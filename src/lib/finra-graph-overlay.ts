...
/** Log-bold base size; multiplied by zoom so bold grows/shrinks with the view. */
const OVERLAY_BOLD_LABEL_SIZE_PX = 20;
/** Hard cap for bold labels (static maximum screen size). */
const OVERLAY_BOLD_LABEL_MAX_PX = 44;
...
export function updateOverlay(
	...
	const placeLabel = (n: Node, isLogBoldLabel: boolean) => {
		...
		if (isLogBoldLabel) {
			el.style.fontSize = `${Math.min(OVERLAY_BOLD_LABEL_MAX_PX, OVERLAY_BOLD_LABEL_SIZE_PX * zoom)}px`;
		} else {
			el.style.fontSize = `${OVERLAY_DEFAULT_LABEL_SIZE_PX * zoom}px`;
		}
