...
export async function GET(request: NextRequest) {
	try {
		...
		if (!skipLocalIndexSearch) {
			data = await searchLocalIndexMany('sec', 'individual', rawQuery, { limit, offset, baseUrl });
		}
		...
		if (allResponses.length > 0) {
			const merged = mergeLocalSearchResponses(allResponses, { bucket: 'sec:individual', limit, offset });
